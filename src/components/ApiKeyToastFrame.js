 ...
 */
export const API_KEY_TOAST_TIMEOUT_MS = 8000;
 ...
 */
export const API_KEY_TOAST_Z_INDEX = 6000;
 ...
 */
export const apiKeyToastFrameStyle = {
  position: 'fixed',
  ...
  zIndex: API_KEY_TOAST_Z_INDEX,
  width: 'min(calc(100vw - var(--pf-t--global--spacer--xl)), 22rem)',
  maxWidth: '100%',
  pointerEvents: 'auto'
};

function useApiKeyToastAutoDismiss(onClose, timeoutMs = API_KEY_TOAST_TIMEOUT_MS) {
  useEffect(() => {
...
/**
 * Wrapper for API key toasts: placement below masthead + auto-dismiss after interval.
 */
const ApiKeyToastFrame = ({ 'aria-label': ariaLabel, onClose, timeoutMs, children }) => {
  useApiKeyToastAutoDismiss(onClose, timeoutMs ?? API_KEY_TOAST_TIMEOUT_MS);

  const node = (
    <div role="region" aria-label={ariaLabel} style={apiKeyToastFrameStyle}>
      {children}
    </div>
  );
...
export default ApiKeyToastFrame;
